...
7. **Domain README** — After any write to a spec, regenerate `docs/specs/{domain}/README.md` for the affected domain(s). The README is fully generated (not hand-edited). See Domain README Format below.

## Domain README Format

When regenerating `docs/specs/{domain}/README.md`, produce the following format. The README is fully machine-generated — never hand-edit it.

1. **Read all specs** in `docs/specs/{domain}/*.md` (files with `type: spec` frontmatter)
2. **Extract** from each spec: `title`, `status`, `acceptance_criteria` (count by status), and the first paragraph of the body (or the `## Overview` section if present) as a one-line summary
3. **Write** `docs/specs/{domain}/README.md` in this format:
...
**Rules:**
- `{Domain Name}` is the domain directory name, title-cased (e.g., `workflow` → `Workflow`, `data-pipeline` → `Data Pipeline`)
- Capabilities are listed alphabetically by filename
- AC counts come from the spec's `acceptance_criteria` frontmatter: count entries by `status` field (`met`, `pending`, `unmet`)
- The summary line is the first paragraph of the spec body after the H1 title (or the `## Overview` section content if present), truncated to one line
- Omit AC breakdown items that are zero (e.g., if no unmet ACs, don't show "0 unmet")
- If the domain has no specs, do not create a README
