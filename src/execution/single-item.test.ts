...
import type { PhaseExecutor, PhaseResult, PhaseOptions } from "../core/phase-executor.js";

const mockRunPhase = vi.fn<(phase: string, input: string, options?: PhaseOptions) => Promise<PhaseResult>>();
...
describe("executeSingleItem", () => {
  ...
  it("runs phases from discover through done", async () => {
    // Arrange
    mockRunPhase.mockResolvedValue(mockPhaseResult());

    // Act
    const result = await executeSingleItem(mockExecutor,"user auth", {
      fromPhase: "discover",
      throughPhase: "done",
    });

    // Assert — should call runPhase for each phase
    expect(mockRunPhase.mock.calls.length).toBe(7);
  ...
  it("respects fromPhase and throughPhase range", async () => {
    ...
    // Act
    await executeSingleItem(mockExecutor,"docs/backlog/P0-item.md", {
      fromPhase: "design",
      throughPhase: "deliver",
    });

    // Assert — only design and deliver
    const phases = mockRunPhase.mock.calls.map((c) => c[0]);
  ...
  it("accumulates total cost across phases", async () => {
    ...
    // Act
    const result = await executeSingleItem(mockExecutor,"docs/backlog/P0-item.md", {
      fromPhase: "design",
      throughPhase: "deliver",
    });
  ...
  it("passes session IDs for cross-phase resume", async () => {
    ...
    // Act
    await executeSingleItem(mockExecutor,"user auth", {
      fromPhase: "discover",
      throughPhase: "define",
    });
  ...
  it("stops review cycles at max limit", async () => {
    ...
    expect(mockRunPhase.mock.calls.length).toBeLessThanOrEqual(3);
  });
...
describe("executeSingleItem cost logging", () => {
  ...
  it("passes hooks to runPhase when logDir is set", async () => {
    ...
    // Assert
    const phaseOpts = mockRunPhase.mock.calls[0][2];
...
describe("executeSingleItem auth mode", () => {
  ...
  it("passes authMode to runPhase when set", async () => {
    ...
    // Act
    await executeSingleItem(mockExecutor,"item.md", {
      fromPhase: "deliver",
      throughPhase: "deliver",
      authMode: "oauth",
    });
...
describe("executeSingleItem artifact tracking", () => {
  ...
  it("records Write tool_use file paths as artifacts", async () => {
    ...
    mockRunPhase.mockImplementation(async (_phase, _input, opts) => {
    ...
    // Act
    const result = await executeSingleItem(mockExecutor,"item.md", {
      fromPhase: "deliver",
      throughPhase: "deliver",
    });
